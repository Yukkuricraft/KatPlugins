package net.katsstuff.bukkit.homesweethome.lib

object LibPerm {

  final val Root = "homesweethome"

  final val Admin  = s"$Root.admin"
  final val Reload = s"$Admin.reload"
  final val Import = s"$Admin.import"
  final val Export = s"$Admin.export"

  final val Mod    = s"$Root.mod"
  final val Search = s"$Mod.search"
