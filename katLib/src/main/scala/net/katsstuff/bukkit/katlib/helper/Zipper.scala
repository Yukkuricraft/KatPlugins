package net.katsstuff.bukkit.katlib.helper

//noinspection MutatorLikeMethodIsParameterless
case class Zipper[+A](lefts: Seq[A], focus: A, rights: Seq[A]) {
  def left: Option[Zipper[A]] =
    if lefts.nonEmpty then Some(Zipper(lefts.init, lefts.last, focus +: rights)) else None

  def right: Option[Zipper[A]] =
    if rights.nonEmpty then Some(Zipper(lefts :+ focus, rights.head, rights.tail)) else None

  def first: Option[Zipper[A]] =
    if lefts.nonEmpty then Some(Zipper(Nil, lefts.head, lefts.tail ++ (focus +: rights))) else None
  ...
  def leftN(amount: Int): Option[Zipper[A]] =
    if amount == 0 then Some(this)
    else if amount < 0 then rightN(-amount)
    else if lefts.lengthCompare(amount) >= 0 then
  ...
  def rightN(amount: Int): Option[Zipper[A]] =
    if amount == 0 then Some(this)
    else if amount < 0 then leftN(-amount)
    else if rights.lengthCompare(amount) >= 0 then
